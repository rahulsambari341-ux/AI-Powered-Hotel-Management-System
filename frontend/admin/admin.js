...
const API_BASE =
  window.HOTEL_API_BASE || "http://127.0.0.1:8000";
...
async function adminLogin() {
  ...
  if (!input || !error || !button) return;

  const token = input.value.trim();

  if (!token) {
    error.textContent = "Please enter the admin token.";
  ...
  try {
    ...
    if (!valid) {
      error.textContent = "Invalid admin token.";
      error.style.display = "block";
      return;
    }
...
function renderCustomers(customers) {
  ...
  body.innerHTML = customers
    ...
    <tr>
      ...
      <td class="mono">${c.booking_count ?? 0}</td>
    </tr>
  `
    )
    .join("");
...
// =========================================================
// STEP 2 & 3 - CLICKABLE STAT CARD DETAILS
// & SEPARATE LISTS
// =========================================================
...
async function loadFilter(filterName) {
  ...
  if (!filterPanel || !filterTitle || !filterHead || !filterBody) {
    return;
  }
...
// =========================================================
// ATTACH CLICK EVENTS TO DASHBOARD CARDS
// =========================================================
...
async function loadDashboard() {
  ...
  try {
    const [
      ...
      fetchJSON(
        "/admin/bookings/recent?limit=50"
      ),
...
function adminModify(
  ...
  if (!modal) {
    // Prompts asking Name, Phone, Email,
    // Adults, Children, Room ID,
    // Check-in, Check-out sequentially

    const newName = prompt(
      `Modify Customer Name for ${bookingId}:`,
      customerName || ""
    );
    ...
    const newPhone = prompt(
      `Modify Customer Phone for ${bookingId}:`,
      ""
    );
    ...
    const newEmail = prompt(
      `Modify Customer Email for ${bookingId}:`,
      ""
    );

    if (newEmail === null) return;

    const newAdults = prompt(
      `Modify Adults count for ${bookingId}:`,
      "1"
    );

    if (newAdults === null) return;

    const newChildren = prompt(
      `Modify Children count for ${bookingId}:`,
      "0"
    );
    ...
    const newRoomId = prompt(
      `Modify Room ID for ${bookingId}:`,
      ""
    );

    if (newRoomId === null) return;

    const newCheckIn = prompt(
      `Modify Check-In date for ${bookingId} (YYYY-MM-DD):`,
      checkIn || ""
    );
    ...
    const newCheckOut = prompt(
      `Modify Check-Out date for ${bookingId} (YYYY-MM-DD):`,
      checkOut || ""
    );
    ...
    fetch(
      `${API_BASE}/admin/bookings/${bookingId}/update`,
      {
        method: "PUT",
  ...
  if (idEl) {
    idEl.value = bookingId;
  }

  if (nameEl) {
    nameEl.value = customerName || "";
  }
...
async function submitEditBooking() {
  ...
  const room_id = room_id_val
    ? parseInt(room_id_val, 10)
    : null;
...
async function openNewBookingModal() {
  ...
  try {
    ...
    select.innerHTML =
      '<option value="">Failed to load rooms</option>';
  }
}
...
async function submitNewBooking() {
  ...
  if (
    ...
    alert(
      "Please select a valid available room from the dropdown."
    );
  ...
  if (
    ...
    alert(
      "Invalid total amount. Please verify your selected dates and room."
    );
  ...
  try {
    const res = await fetch(
      `${API_BASE}/admin/bookings/create`,
      {
        method: "POST",
    ...
    alert(
      `Booking created successfully! ID: ${data.booking_id}`
    );
